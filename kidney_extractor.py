"""Extract kidney function dose adjustment information from pharmaceutical monographs."""
...
from .config import client, SAFE_DELAY, KIDNEY_OUTPUT_FOLDER
from .pdf_utils import extract_pdf_text
from .api_client import call_ai_api
from .prompts import KIDNEY_DOSE_EXTRACTION_PROMPT, KIDNEY_SYSTEM_MESSAGE
...
def extract_kidney_dose_adjustment_from_pdf(pdf_path: str, filename: str) -> str:
    """
    Extract dose adjustment information for kidney function from a single PDF file.
    ...
    Returns:
        String with dose adjustment recommendation for kidney
    """
    try:
        ...
        # Extract kidney dose adjustment info
        prompt = KIDNEY_DOSE_EXTRACTION_PROMPT.format(dosage_section_text=dosage_section)
        result = call_ai_api(prompt, KIDNEY_SYSTEM_MESSAGE, filename, temperature=0.1)
        ...
        if result:
            ...
            valid_options = [
                ...
                "No dose adjustment required for kidney impairment",
                "Dose adjustment recommended with kidney impairment",
        ...
        print(f"    ❌ Error in extract_kidney_dose_adjustment_from_pdf: {e}")
        return f"ERROR: {str(e)}"

def process_kidney_folder(target_folder):
    """Process all PDFs and extract dose adjustment information for kidney only."""
    ...
    print(f"📂 Processing Folder: {folder_name} ({len(all_files)} files) - KIDNEY ONLY")
    ...
    for idx, filename in enumerate(all_files, 1):
        ...
        kidney_info = extract_kidney_dose_adjustment_from_pdf(file_path, filename)
        
        folder_dataset.append({
            "ID": file_id,
            "Kidney Function Dose Adjustment": kidney_info
        })
        
        print(f"\n  {'-'*40}")
        print(f"  ✅ FINISHED: {filename}")
        print(f"     Kidney: {kidney_info}")
        print(f"  {'-'*40}")
    ...
    if folder_dataset:
        if not os.path.exists(KIDNEY_OUTPUT_FOLDER):
            os.makedirs(KIDNEY_OUTPUT_FOLDER)
            print(f"\n  📁 Created folder: {KIDNEY_OUTPUT_FOLDER}")

        df = pd.DataFrame(folder_dataset)
        output_filename = os.path.join(KIDNEY_OUTPUT_FOLDER, f"{folder_name}_kidney_dose_adjustment.xlsx")
...
if __name__ == "__main__":
    ...
    print("\n✨ Kidney dose adjustment extraction complete!")